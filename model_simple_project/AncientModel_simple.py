...
class ResBlock(nn.Module):
    def __init__(self, in_channel, out_channels, s=1):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels=in_channel, out_channels=out_channel_1, kernel_size=1),
            nn.BatchNorm2d(out_channel_1),
            nn.ReLU())

        self.conv2 = nn.Sequential(
            nn.Conv2d(in_channels=out_channel_1, out_channels=out_channel_2, kernel_size=3, padding=1),
            nn.BatchNorm2d(out_channel_2),
            nn.ReLU())

        self.conv3 = nn.Sequential(
            nn.Conv2d(in_channels=out_channel_2, out_channels=in_channel, kernel_size=1),
